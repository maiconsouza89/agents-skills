...
- **Triagem num passo:** labels são a API de escrita dos três campos. `pnpm triage-issue <N> [--priority <p0|p1|p2|backlog>] [--area <cli|core|site|ci|catalog>] [--complexity <low|medium|high>]` (`tools/triage-issue.ts`) grava os labels `priority:*`, `area:*` e `complexity:*` por caminhos `repos/...`, então funciona igual em sessão local e web, sem dispatch. Classifique antes com as skills `mass-issue-priority` e `mass-issue-complexity`.
- `project-fields.yml` reage a `issues.labeled` e espelha cada label no campo correspondente do Project, removendo os labels irmãos da mesma família: o último aplicado vence. Vale para qualquer caminho que aplique o label — o comando, a interface do GitHub ou a rotina de triagem.
...
- A rotina `Triage issue` (Claude Code web, gatilho `issues.opened`) faz a mesma triagem sozinha: classifica pelas duas skills, grava os três labels e comenta as justificativas. O texto do prompt é versionado em `tools/routines/triage-issue.md`; a plataforma não lê o arquivo, então toda mudança nele só vale depois de colar o texto novo na rotina.

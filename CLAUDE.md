...
- `security-scan.yml` (`main`, PRs e `workflow_dispatch`): `tools/allowlist.ts` roda sempre e falha com entrada vencida; `uvx snyk-agent-scan@latest $TARGETS --ci` só em push e PR do próprio repo (precisa de `SNYK_TOKEN`). Num PR, `$TARGETS` vem de `tools/changed-skills.ts`: só as skills cujo `contentHash` mudou no `skills-registry.json` contra a base, mais qualquer skill com arquivo tocado no diff (rede de segurança para registry desatualizado); se nada mudou, o Snyk nem roda. Em push para `main` e no dispatch manual o alvo é o catálogo inteiro (`skills`). Não há filtro `paths:` no trigger porque `scan` é check obrigatório e workflow pulado por filtro de path nunca reporta o check. O erro `X007` (limite diário da versão pública do Snyk Agent-Scan) é tratado como não-bloqueante — o step captura a saída, e só falha o job se o código de saída for diferente de zero *e* a saída não contiver `X007`; qualquer outra falha (achado real, erro de auth, etc.) continua bloqueando.
...
- `release.yml` (tag `v*`): `pnpm check`, `pnpm build`, depois publica `@mass-solutions/skills-core` e `@mass-solutions/skills-cli` no npm com `pnpm -r publish --access public --provenance --no-git-checks` e cria o GitHub Release (`gh release create --generate-notes`). **Ninguém publica no npm à mão**: o CLI `0.1.0` saiu com `workspace:*` por isso. Versões dos pacotes via Changesets (`pnpm changeset`, `pnpm changeset version`); o site não é versionado.
- A autenticação no npm é trusted publishing (OIDC): o job tem `id-token: write` e nenhum secret de registry. Cada pacote precisa de um Trusted Publisher configurado em npmjs.com (GitHub Actions, owner `maiconsouza89`, repo `agents-skills`, workflow `release.yml`, environment em branco); sem isso o publish falha com 404. Tokens npm de escrita expiram em 90 dias e exigem 2FA, por isso não são usados. O `--provenance` anexa a attestation SLSA; o pnpm 11 faz a troca OIDC nativamente (exige `pnpm >= 11.1.2`).
- Antes de publicar, o job aborta se a tag for diferente de `v<versão de packages/cli/package.json>` ou se algum tarball empacotado ainda contiver `workspace:`. `pnpm -r publish` pula versões já no registry e o Release só é criado se não existir, então rerun da mesma tag é seguro.
- O CLI baixa o catálogo por padrão em `v${version}` (`DEFAULT_REF` em `packages/cli/src/download.ts`), então a tag precisa ser exatamente `v<versão do CLI>` no commit que fez o bump: `pnpm changeset version`, `pnpm check`, `pnpm build`, PR com o bump mergeado em `main`, depois `git tag v<versão> && git push origin v<versão>`. Uma versão publicada sem a tag correspondente quebra `install`, `update` e `search` para quem não passar `--ref`.
...
- **Por que a escrita no Project passa por Actions:** o proxy do GitHub nas sessões do Claude Code web bloqueia GraphQL inteiro e qualquer REST fora de `repos/<owner>/<repo>/...` (inclui os endpoints de Projects v2 de usuário), sempre com 403. Por isso os campos são escritos por workflows, com o secret `PROJECT_TOKEN` (PAT clássico, escopo `project` — único tipo de credencial que alcança um Project de usuário).
- **Ao começar uma issue, rode `pnpm start-issue <N>` antes de qualquer outra coisa.** Ele cria ou reaproveita a branch e atribui a issue; a atribuição dispara `project-board.yml`, que move o item para `In Progress`. Sem esse comando o board não é atualizado. Se esqueceu, rode assim que perceber: ele funciona com a branch já criada. Numa sessão web ele mantém a branch da sessão e só atribui.
- **Triagem:** `triage.yml` roda quando uma issue é aberta. O `claude-code-action` classifica só com `Read` (a issue em `issue.json`, as rubricas das skills `mass-issue-priority` e `mass-issue-complexity`, a regra de Area abaixo) e devolve JSON via `--json-schema`; um step em shell valida cada valor contra as opções do board, grava os três campos com `PROJECT_TOKEN` e comenta `Triage: ...` com as justificativas. Um valor fora das opções não grava nada e derruba o run. Para triar de novo: "Re-run workflow" na aba Actions, ou `workflow_dispatch` com o número (`gh workflow run triage.yml -f issue=<N>`; numa sessão web, `POST repos/<owner>/<repo>/actions/workflows/triage.yml/dispatches`). Ajuste manual é nos dropdowns do board.
...
- Os comandos compartilham helpers em `tools/lib/gh.ts`; se `gh` estiver ausente (acontece nalgumas sessões cloud), caem para `fetch` com `GH_TOKEN`/`GITHUB_TOKEN`, e sem nenhum dos dois orientam usar o GitHub MCP.

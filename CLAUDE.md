...
- O workflow `project-board.yml` adiciona toda issue aberta ao Project #5 e move o Status para `In Progress` quando alguém é atribuído, usando o secret `PROJECT_TOKEN` (PAT clássico com escopo `project`; é o único tipo de credencial que alcança um Project de usuário — nem `GITHUB_TOKEN` nem fine-grained PAT/GitHub App conseguem). Essa automação vive em Actions, e não em `start-issue`, porque o proxy do GitHub nas sessões cloud do Claude Code bloqueia GraphQL (Projects v2 é GraphQL-only) **e também** qualquer caminho REST que não seja escopado por repositório (`repos/<owner>/<repo>/...`) — confirmado ao vivo: `users/<owner>/projectsV2/...` leva 403 do próprio proxy antes de chegar ao GitHub, com token nenhum resolvendo isso.
...
- Complexidade: classifique a issue com a skill `mass-issue-complexity` e grave o resultado com `pnpm set-complexity <N> <low|medium|high>` (`tools/set-complexity.ts`). Local, escreve direto via REST (`PATCH users/<owner>/projectsV2/<n>/items/<item>`). Numa sessão do Claude Code web esse caminho leva 403 (path não escopado por repositório), então o comando dispara o workflow `set-complexity.yml` via `POST repos/<owner>/<repo>/actions/workflows/set-complexity.yml/dispatches` (esse sim escopado por repositório, passa no proxy) — o workflow faz a escrita de verdade num runner, com o mesmo `PROJECT_TOKEN`. Sem opção `--owner`/`--project`, usa o Project #5 do dono do remoto `origin`. Não escreve label nenhuma — o campo `Complexity` do Project é a única fonte.
- `tools/lib/gh.ts` concentra os helpers reusados por `start-issue.ts` e `set-complexity.ts` (`ownerAndName`, e `ghApi`/`ghApiPaginated` com o fallback REST-via-`fetch` quando `gh` está ausente).

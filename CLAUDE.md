...
- Complexidade: classifique a issue com a skill `mass-issue-complexity` e grave o resultado com `pnpm set-complexity <N> <low|medium|high>` (`tools/set-complexity.ts`). Diferente do Status, esse comando roda direto numa sessão do Claude Code web: ele usa a API REST de Projects v2 (`PATCH users/<owner>/projectsV2/<n>/items/<item>`), não GraphQL, então não esbarra no bloqueio do proxy. Sem opção `--owner`/`--project`, usa o Project #5 do dono do remoto `origin`. Não escreve label nenhuma — o campo `Complexity` do Project é a única fonte.

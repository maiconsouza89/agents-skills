...
Appended by builder B2 (S5):

- **Boundary:** C43-C60 e C82 fechados em `2b9e767` (um commit: `packages/cli/{bin,src,test}`, `packages/core/src/index.ts` + bins, `AGENTS.md`). Verificado também com o binário real (`packages/cli/bin/mass-skills.js`) contra um `python3 -m http.server` servindo o repo: list, install, update --check, doctor (exit 1 com edição local), update --force, doctor (exit 0), remove.
- **Settled mid-build:** `-a` tem default `auto` (AC 49 já define o comportamento sem agente detectado, então omitir `-a` não é erro de uso). Nome de skill fora do regex vindo do usuário recebe o mesmo `Unsafe path` + exit `1` do AC 51, antes de qualquer download; nome válido mas ausente do registry é exit `2` (AC 50). `contentHash` divergente imprime `Integrity check failed for <name>: contentHash`. `update` classifica cada entrada olhando o primeiro agente registrado cujo diretório existe; `doctor` olha todos os agentes registrados, com prioridade missing > modified > deprecated > ok. `remove` de skill fora do lockfile é exit `2`. O core ganhou `validateMain`/`registryMain` no index e os bins aceitam qualquer `{ write }` (antes inferiam `typeof process.stdout`, o que quebrava o `tsc` do CLI). `commander` 15 com `exitOverride` + `configureOutput`; erros do commander viram exit `2`, help vira `0`. Sem door nova; `Flow` e `Impact` continuam verdadeiros.
- **Abandoned:** `rename` direto do tmpdir para o agente (falha entre filesystems: `/tmp` vs projeto) - substituído por `cpSync` para um irmão oculto no diretório do agente e `rename` por cima; subpath export `@mass-solutions/skills-core/bin/*` (exigiria alias extra no vitest) - substituído pelos re-exports no index.

Appended by builder B3 (S6-S8):

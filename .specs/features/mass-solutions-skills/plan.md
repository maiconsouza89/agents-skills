...
| Download no CLI | `https://raw.githubusercontent.com/maiconsouza89/agents-skills/<ref>/<path>`, `--ref` default `main`, timeout 30 s por arquivo | Decisions do discovery | y |
...
| Tema do site | escuro: `--color-canvas: #201d1d`, `--color-ink: #fdfcfc`, `color-scheme: dark`; o `DESIGN.md` descreve o sistema original em cream e ganha uma nota de adaptação | decisão do usuário em 2026-09-15 (commit `b362a47`), depois do build; AC 70 e C70 renegociados nesta data | y |
| Nome do repositório | `maiconsouza89/agents-skills` | decisão do usuário em 2026-09-15: o repo criado no GitHub tem esse nome; door 9 renegociada | y |
...
| --- | --- | --- | --- |
| 1 | blocks go-live | Go-ahead para criar `maiconsouza89/agents-skills` no GitHub e fazer o primeiro push | nada externo lista as skills; proxies do Success não rodam contra o remoto |
...
48. IF um agente pedido em `-a` não está na tabela da door 10 THEN the system SHALL imprimir `Unsupported agent "<id>". Supported: <lista>. For other agents use: npx skills add maiconsouza89/agents-skills` e sair com exit code `2`
...
61. WHEN `pnpm build` roda THEN the system SHALL gerar em `apps/site/dist/` as páginas `/`, `/skills/<name>/` para cada uma das 5 skills, `/install/`, `/agents/`, `/search-index.json`, `/404.html` e as mesmas sob `/pt-br/` (exceto o índice), com `base` `/agents-skills/` em todos os links internos
...
65. WHEN `/skills/<name>/` renderiza THEN the system SHALL mostrar `description` separada em "what it does", "use when" e "do not use for", o corpo do `SKILL.md` renderizado em inglês nas duas rotas, `metadata` (`author`, `version`, `reviewed`, `category`, `tags`, `requires` se houver), a lista de `files[]` com `bytes`, o `contentHash`, o link do fonte no GitHub e um painel de instalação com os três comandos (`mass-skills install <name> -a <agent>`, `npx skills add maiconsouza89/agents-skills --skill <name>`, `/plugin marketplace add maiconsouza89/agents-skills`) e o aviso de que só o CLI verifica hash
...
70. The system SHALL usar em todo o chrome do site os tokens do `DESIGN.md` com o tema escuro decidido em 2026-09-15: fonte monoespaçada em todos os textos, fundo `#201d1d` (`--color-canvas`), tinta `#fdfcfc` (`--color-ink`), raio `4px` só em elementos interativos, marcadores ASCII `[+]`/`[-]` como bullets, sem sombras e sem gradientes
...
74. The system SHALL conter `.github/workflows/security-scan.yml` disparado em `push` em `main` e em `pull_request`, com o passo Snyk condicionado a `github.event_name == 'push' || github.event.pull_request.head.repo.full_name == github.repository`, executando `uvx snyk-agent-scan@latest skills --ci --dangerously-run-mcp-servers` (a flag é exigida pelo `--ci` da versão em uso, 0.6.3; inerte aqui porque não há MCP config, só um diretório de skills) com `SNYK_TOKEN` e as `--ignore-risks` vindas da allowlist vigente
...
81. WHEN `pnpm registry` roda THEN the system SHALL registrar em `skills-registry.json` o campo `version` do registry como `1` e `repo` como `maiconsouza89/agents-skills`, o que fixa o contrato que o CLI de qualquer versão instalada lê
...
3. out: `dist/` estático publicado pelo `pages.yml` em `https://maiconsouza89.github.io/agents-skills/` (door 9)

Caminho sem o CLI (não passa por código nosso):

1. `npx skills add maiconsouza89/agents-skills` -> lê `skills/` e `.claude-plugin/marketplace.json` (door 1, door 5), instala sem hash
2. out: `/plugin marketplace add maiconsouza89/agents-skills` -> lê `marketplace.json` (door 5), carrega `./skills/`
...
| Route | In | Out | Status |
| --- | --- | --- | --- |
| `GET https://raw.githubusercontent.com/maiconsouza89/agents-skills/<ref>/skills-registry.json` | `ref` (default `main`) | registry JSON (door 3) | `200`, `404` |
| `GET https://raw.githubusercontent.com/maiconsouza89/agents-skills/<ref>/skills/<name>/<path>` | `ref`, `name`, `path` de `files[]` | bytes do arquivo, conferidos contra `sha256` | `200`, `404` |
...
| `GET /`, `/pt-br/`, `/skills/<name>/`, `/pt-br/skills/<name>/`, `/install/`, `/agents/`, `/search-index.json` (sob `base` `/agents-skills/`) | rota | HTML estático; JSON no índice | `200`, `404` |
...
| 9. Nomes públicos (renegociada pelo usuário em 2026-09-15: era `mass-solutions-skills`, o repo criado no GitHub chama-se `agents-skills`) | repo `maiconsouza89/agents-skills`; site `https://maiconsouza89.github.io/agents-skills/` com `base: "/agents-skills/"`, rota PT em `/pt-br/`; escopo npm reservado `@mass-solutions/*`; marketplace `mass-solutions` | org `mass-solutions` no GitHub - não confirmada como do usuário; mudar depois quebra todo comando de instalação já divulgado |
